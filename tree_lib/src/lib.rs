...
impl MerkleTree {
    ...
    pub fn new<T: AsRef<[u8]>>(items: &[T]) -> Self {
        ...
        Self::from_hashed_items(&hashed)
    }

    pub fn from_tree<T: AsRef<[u8]>>(tree: Self, items: &[T]) -> Self {
        let input_leafs = &tree.nodes[..tree.leaf_count];

        let mut hashed = Vec::with_capacity(input_leafs.len() + items.len());
        hashed.extend_from_slice(&input_leafs);
    ...
    fn from_hashed_items(items: &[Hash]) -> Self {
...
mod tests {
    ...
    const TEST_UP: &[&[u8]] = &[b"no", b"errors"];

    const BAD: &[&[u8]] = &[b"bad", b"missing", b"false"];

    #[test]
    fn test_tree_update_valid() {
        let mt = MerkleTree::new(TEST);

        let mt_up = MerkleTree::from_tree(mt, TEST_UP);

        let test_updated_input = [TEST, TEST_UP].concat();

        let mt_up_check = MerkleTree::new(&test_updated_input);

        assert_eq!(mt_up.get_root(), mt_up_check.get_root());
    }
